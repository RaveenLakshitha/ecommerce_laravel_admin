...
<head>
    ...
    <style>
        :root {
            ...
            ;
            /* Default modern dark blue (slate-900) */
            --primary-light:
                {{ adjustBrightness($primary_color ?? '#0f172a', 20) }}
            ;
            --primary-dark:
                {{ adjustBrightness($primary_color ?? '#0f172a', -20) }}
            ;
    ...
    <div class="flex min-h-screen bg-slate-50 relative">
        ...
        <div class="absolute top-5 right-6 lg:top-8 lg:right-10 z-50">
            <div x-data="{ langOpen: false }" class="relative">
                <button @click="langOpen = !langOpen" class="flex items-center space-x-2.5 
                ...
                <div x-show="langOpen" @click.away="langOpen = false" x-transition.opacity.duration.150 class="absolute right-0 mt-3 w-44 
                   ...
                   py-3 overflow-hidden z-50">
                    ...
                    <form action="{{ route('language.switch') }}" method="POST">
                        ...
                        <button type="submit" class="w-full text-left px-5 py-3 text-sm font-medium 
        ...
        <div class="hidden lg:flex lg:w-1/2 image-section relative items-center justify-center overflow-hidden">
            ...
            <div class="absolute top-1/4 left-1/4 w-64 h-64 bg-white/10 rounded-full blur-3xl mix-blend-overlay animate-pulse"
                style="animation-duration: 4s;"></div>
            <div class="absolute bottom-1/4 right-1/4 w-96 h-96 bg-slate-400/20 rounded-full blur-3xl mix-blend-overlay animate-pulse"
                style="animation-duration: 6s;"></div>

            <div class="image-content text-white p-12 max-w-xl w-full">
                <div
                    class="glass-panel p-8 rounded-3xl shadow-2xl transform transition-transform duration-700 hover:scale-[1.02]">
                    <div class="flex items-center space-x-4 mb-8">
                        @if($site_logo ?? false)
                            <img src="{{ $site_logo }}" alt="{{ $site_name ?? 'Admin' }}"
                                class="w-16 h-16 rounded-2xl object-cover shadow-lg ring-4 ring-white/20">
                        @else
                            <div class="w-16 h-16 bg-white rounded-2xl flex items-center justify-center p-3 shadow-lg">
                                <svg class="w-10 h-10 text-slate-800" fill="none" stroke="currentColor" viewBox="0 0 24 24"
                                    ...
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M12 4.354l8 3.556v7.11m-8 6.425l-8-3.556v-7.11"></path>
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                        d="M12 10.5l8-3.556m-8 3.556l-8-3.556m8 3.556v10.944"></path>
                                </svg>
                            </div>
                        @endif
                        <h2 class="text-3xl font-bold tracking-tight">{{ $site_name ?? __('messages.Admin Portal') }}
                        </h2>
                    </div>
                </div>
            </div>
