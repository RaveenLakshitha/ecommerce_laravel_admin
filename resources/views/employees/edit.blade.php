...
@section('title', __('file.edit_employee') . ' - ' . $employee->full_name)
...
@section('content')
    <div class="px-4 sm:px-6 lg:px-4 pb-4 sm:py-12 pt-20">
        ...
        <div class=" flex items-center text-sm text-gray-500 dark:text-gray-400 mb-3">
            ...
            <a href="{{ route('employees.show', $employee) }}"
                ...
                {{ Str::limit($employee->full_name, 25) }}
            </a>
            <svg class="w-4 h-4 mx-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" />
            </svg>
        ...
        <h1 class="text-3xl font-semibold text-gray-900 dark:text-white">{{ __('file.edit_employee') }}</h1>
        <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('file.update_employee_record') }}</p>
    </div>

    <form method="POST" action="{{ route('employees.update', $employee) }}" class="space-y-8" enctype="multipart/form-data">
        @csrf
        @method('PUT')
                    ...
                    <div class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                ...
                                @error('first_name') <p class="mt-1.5 text-xs text-red-600 dark:text-red-400">{{ $message }}
                                </p> @enderror
                            </div>
                            <div>
                                <label
                            ...
                            <div>
                                ...
                                @error('last_name') <p class="mt-1.5 text-xs text-red-600 dark:text-red-400">{{ $message }}
                                </p> @enderror
                            </div>
                        </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <select name="user_id" id="user_id_select"
                                        class="w-full px-3 py-2.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-gray-100 focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all">
                                        <option value="">{{ __('file.select_existing_user') }}</option>
                                        @foreach($users as $user)
                                            <option value="{{ $user->id }}"
                                                data-phone="{{ $user->phone ? e($user->phone) : '' }}" {{ old('user_id', $employee->user_id) == $user->id ? 'selected' : '' }} {{ $user->employee && $user->id !== $employee->user_id ? 'disabled' : '' }}>
                                                {{ $user->name }} ({{ $user->email }})
                                                @if($user->employee && $user->id !== $employee->user_id)
                                    ...
                                    <div class="flex items-center gap-4">
                                        ...
                                        <div>
                                            <input type="file" name="photo" accept="image/*" onchange="previewImage(this)"
                                                class="block w-full text-sm text-gray-500 dark:text-gray-400 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-sm file:font-medium file:bg-gray-900 file:text-white hover:file:bg-gray-800 dark:file:bg-gray-700 dark:hover:file:bg-gray-600">
                                            <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Max 2MB. JPEG, PNG, GIF
                                            </p>
                                            @error('photo') <p class="mt-1 text-xs text-red-600 dark:text-red-400">
                                                {{ $message }}
                                            </p> @enderror
                                        </div>
                                    </div>
                            ...
                            <div>
                                ...
                                <select name="gender"
                                    ...
                                    @foreach($genders as $gender)
                                        <option value="{{ $gender->slug }}" {{ old('gender', $employee->gender) === $gender->slug ? 'selected' : '' }}>
                                            {{ __($gender->name) }}
                                        </option>
                                    @endforeach
                                </select>
                                @error('gender') <p class="mt-1.5 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="address" rows="3"
                                ...
                                placeholder="{{ __('file.address_placeholder') }}">{{ old('address', $employee->address) }}</textarea>
                            @error('address') <p class="mt-1.5 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <div>
                                ...
                                @error('city') <p class="mt-1.5 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                            <div>
                                <label
                                ...
                                @error('state') <p class="mt-1.5 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                                @enderror
                            </div>
                            <div>
                                <label
                        ...
                        <div>
                            ...
                            @error('country') <p class="mt-1.5 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                ...
                                <div>
                                    ...
                                    <input type="text" name="emergency_contact_phone"
                                        value="{{ old('emergency_contact_phone', $employee->emergency_contact_phone) }}" minlength="7" maxlength="15"
                                        class="w-full px-2 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-500 focus:border-transparent dark:bg-transparent dark:text-white transition-shadow"
                            ...
                            <div>
                                ...
                                <select name="reporting_to"
                                    ...
                                    @foreach($supervisors as $supervisor)
                                        <option value="{{ $supervisor->id }}" {{ old('reporting_to', $employee->reporting_to) == $supervisor->id ? 'selected' : '' }}>
                ...
                <div id="content-professional" class="tab-content hidden">
                    <div class="space-y-6">
                        <div>
                            ...
                            <input type="text" name="profession" value="{{ old('profession', $employee->profession) }}"
                                class="w-full px-2 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-500 focus:border-transparent dark:bg-transparent dark:text-white transition-shadow"
                        ...
                        <div>
                            ...
                            <textarea name="professional_bio" rows="4"
                                class="w-full px-2 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-500 focus:border-transparent dark:bg-transparent dark:text-white transition-shadow resize-none">{{ old('professional_bio', $employee->professional_bio) }}</textarea>
                        ...
                        <div class="pt-6 border-t border-gray-200 dark:border-gray-700">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                                <div>
                                    ...
                                    <input type="text" name="degree" value="{{ old('degree', $employee->degree) }}"
                                        class="w-full px-2 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-500 focus:border-transparent dark:bg-transparent dark:text-white transition-shadow"
                                ...
                                <div>
                                    ...
                                    <input type="text" name="institution"
                                        value="{{ old('institution', $employee->institution) }}"
                                        class="w-full px-2 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-500 focus:border-transparent dark:bg-transparent dark:text-white transition-shadow"
                                ...
                                <div>
                                    ...
                                    <input type="number" name="year_completed"
                                        value="{{ old('year_completed', $employee->year_completed) }}" min="1900"
                                        max="{{ date('Y') + 10 }}"
                                        class="w-full px-2 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-500 focus:border-transparent dark:bg-transparent dark:text-white transition-shadow"
                                        placeholder="2020">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    ...
                                    <input type="text" name="license_type"
                                        value="{{ old('license_type', $employee->license_type) }}"
                                        class="w-full px-2 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-500 focus:border-transparent dark:bg-transparent dark:text-white transition-shadow"
                                ...
                                <div>
                                    ...
                                    <input type="date" name="license_issue_date"
                                        value="{{ old('license_issue_date', $employee->license_issue_date?->format('Y-m-d')) }}"
                                        class="w-full px-2 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-500 focus:border-transparent dark:bg-transparent dark:text-white transition-shadow [color-scheme:light] dark:[color-scheme:dark]">
                                ...
                                <div>
                                    ...
                                    <input type="date" name="license_expiry_date"
                                        value="{{ old('license_expiry_date', $employee->license_expiry_date?->format('Y-m-d')) }}"
                                        class="w-full px-2 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-500 focus:border-transparent dark:bg-transparent dark:text-white transition-shadow [color-scheme:light] dark:[color-scheme:dark]">
                                ...
                                <div class="md:col-span-2">
                                    ...
                                    <input type="text" name="license_issuing_authority"
                                        value="{{ old('license_issuing_authority', $employee->license_issuing_authority) }}"
                                        class="w-full px-2 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-500 focus:border-transparent dark:bg-transparent dark:text-white transition-shadow"
                ...
                <div id="content-employment" class="tab-content hidden">
                    <div class="space-y-6">
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                <label
                                    class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('file.employee_code') }}</label>
                                <input type="text" name="employee_code" value="{{ $employee->employee_code }}" readonly
                                    class="w-full px-2 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg bg-gray-100 dark:bg-gray-800 cursor-not-allowed focus:ring-0">
                                <p class="mt-1 text-xs text-gray-500 dark:text-gray-400">Auto-generated – cannot be changed
                                </p>
                            </div>
                        </div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <input type="text" name="payment_frequency"
                                    value="{{ old('payment_frequency', $employee->payment_frequency) }}"
                                    class="w-full px-2 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-500 focus:border-transparent dark:bg-transparent dark:text-white transition-shadow"
    ...
    <script>
        // Same tab switching function as in create page
        ...
        // Optional: phone auto-fill when selecting user (same as create)
        function fillUserData() {
            const select = document.getElementById('user_id_select');
            if (!select) return;

            const phoneInput = document.querySelector('input[name="emergency_contact_phone"]'); // or separate phone field if you have one
