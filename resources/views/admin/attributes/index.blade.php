...
@section('title', 'Attributes & Options')
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    Manage product attributes (e.g. Size, Color)
                </p>
            </div>
                ...
                <table id="application-table" class="w-full divide-y divide-gray-200 dark:divide-surface-tonal-a30 nowrap"
                    ...
                    <thead
                        ...
                        <tr>
                            ...
                            <th
                                class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider all">
                                Name</th>
                            <th
                                class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider desktop">
                                Type</th>
                            <th
                                class="px-6 py-4 text-left text-xs font-semibold text-gray-500 uppercase tracking-wider desktop">
                                Values Count</th>
                            <th
                                class="px-6 py-4 text-right text-xs font-semibold text-gray-500 uppercase tracking-wider all">
                                Actions</th>
                        </tr>
                    </thead>
                    <tbody
                        class="bg-white dark:bg-surface-tonal-a20 divide-y divide-gray-200 dark:divide-gray-700 [&>tr]:group">
                    </tbody>
                </table>
            </div>
        </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                const table = $('#application-table').DataTable({
                    ...
                    responsive: false,
                    ajax: {
                        url: '{{ route('attributes.datatable') }}'
                    },
                    order: [[0, 'desc']],
                    columnDefs: [
                            ...
                            render: function (data, type, row) {
                                return `
                                            <div class="flex items-center justify-end gap-3 transition-opacity">
                                                <a href="${row.edit_url}" class="text-indigo-600 hover:text-indigo-900 dark:text-indigo-400 p-1.5 rounded-lg hover:bg-indigo-50 dark:hover:bg-indigo-900/20" title="Edit & Options">
                    ...
                    language: {
                        ...
                        emptyTable: "No attributes found matching your criteria.",
                        processing: false,
                    },
                    autoWidth: false,
                    scrollX: false
                ...
                $('#bulk-delete-form-el').on('submit', function (e) {
                    e.preventDefault();
                    if (!confirm('Are you sure you want to delete the selected items and all their options?')) return;
                ...
                window.confirmDelete = function (url) {
                    if (!confirm('Are you sure you want to delete this attribute and all its options?')) return;
