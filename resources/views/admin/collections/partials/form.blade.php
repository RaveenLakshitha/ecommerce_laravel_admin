<form id="collection-drawer-form"
    action="{{ $collection ? route('collections.update', $collection->id) : route('collections.store') }}" method="POST"
    ...
    <div class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="name" id="drawer_col_name"
                value="{{ $collection ? $collection->name : old('name') }}" required
        ...
        <div>
            ...
            <input type="text" name="slug" id="drawer_col_slug"
                value="{{ $collection ? $collection->slug : old('slug') }}"
        ...
        <div class="grid grid-cols-2 gap-4">
            <div>
                ...
                <input type="date" name="start_date" id="drawer_col_start"
                    value="{{ ($collection && $collection->start_date) ? $collection->start_date->format('Y-m-d') : old('start_date') }}"
            ...
            <div>
                ...
                <input type="date" name="end_date" id="drawer_col_end"
                    value="{{ ($collection && $collection->end_date) ? $collection->end_date->format('Y-m-d') : old('end_date') }}"
                    class="block w-full rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50/100 dark:bg-surface-tonal-a20 px-4 py-2.5 text-sm font-normal shadow-sm text-gray-900 dark:text-white outline-none transition-all focus:bg-white dark:focus:bg-surface-tonal-a30 focus:border-primary/50 focus:ring-4 focus:ring-primary/5">
            </div>
        </div>
        ...
        <div>
            ...
            <div class="relative w-full rounded-xl overflow-hidden border-2 border-dashed border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-surface-tonal-a10 cursor-pointer group/upload hover:border-primary transition-all aspect-[21/9]"
                ...
                <input type="file" name="banner_image" id="drawer-banner-input" class="hidden" accept="image/*"
                    onchange="previewDrawerBanner(this)">
            </div>
    ...
    <div class="flex gap-3 pt-6 border-t border-gray-100 dark:border-white/5">
        ...
        <button type="submit"
            ...
            <span
                id="collection-drawer-save-text">{{ $collection ? __('file.save_changes') : __('file.save_collection') }}</span>
            <div id="collection-drawer-loader"
