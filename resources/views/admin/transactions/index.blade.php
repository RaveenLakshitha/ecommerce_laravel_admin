    ...
    <div class="admin-page animate-fade-in-up">
        <div class="admin-page-inner">
            ...
            <nav class="admin-breadcrumb mt-6" aria-label="Breadcrumb">
                ...
                <span class="active">{{ __('file.payment_transactions') }}</span>
            </nav>

            <div class="admin-page-header">
                <div>
                    <h1 class="admin-page-title">{{ __('file.payment_transactions') }}</h1>
                    <p class="admin-page-subtitle">{{ __('file.manage_transactions') }}</p>
                </div>
            </div>
            ...
            <div id="bulk-delete-form" class="hidden animate-fade-in-scale sticky top-20 z-30 mb-6">
                <form method="POST" action="{{ route('transactions.bulkDelete') }}" id="bulk-delete-form-el"
                    ...
                    <div class="flex items-center gap-3">
                        ...
                        <span>{{ __('file.transactions_selected') ?? 'Transactions Selected' }}</span>
                    </div>
                    <button type="submit"
                ...
                const table = $('#application-table').DataTable({
                    ...
                    columns: [
                        ...
                        },
                        {
                            ...
                            render: data => `<span class="text-sm text-gray-400 font-medium">${data}</span>`
                        },
                        {
                            data: 'order_number_html', name: 'order.order_number',
                            render: data => `<span class="text-sm font-bold text-indigo-600 dark:text-indigo-400 uppercase leading-tight">${data}</span>`
                        },
                        {
                        ...
                        },
                        {
                            ...
                            render: data => `<span class="admin-badge admin-badge-neutral text-[10px] font-bold uppercase tracking-wider italic">${data}</span>`
                        },
                        { data: 'status_html', name: 'status', className: 'text-center' },
                        {
                            data: 'amount_html', name: 'amount', className: 'text-right',
                            render: data => `<span class="text-sm font-black text-gray-900 dark:text-white italic">${data}</span>`
                        },
                        {
                            data: null,
                            ...
                            render: function (data, type, row) {
                                return `
                                                <div class="flex items-center justify-end gap-1.5 px-3">
                                                    <a href="${row.show_url}" class="p-1.5 rounded-lg text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-all" title="Inspect">
