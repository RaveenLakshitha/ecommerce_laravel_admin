<form action="{{ $rate ? route('shipping.rates.update', $rate->id) : route('shipping.rates.store') }}" method="POST"
    id="rate-drawer-form" enctype="multipart/form-data" class="space-y-6">
    @csrf
    @if($rate)
        @method('PUT')
    @endif
    ...
    <div class="space-y-4">
        ...
        <div>
            ...
            <input type="number" step="0.01" name="rate_amount"
                value="{{ old('rate_amount', $rate ? $rate->rate_amount : '') }}" required
                class="block w-full rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50/100 dark:bg-surface-tonal-a20 px-4 py-2.5 text-sm font-normal shadow-sm placeholder:text-gray-400 dark:placeholder:text-gray-500 text-gray-900 dark:text-white outline-none transition-all focus:bg-white dark:focus:bg-surface-tonal-a30 focus:border-primary/50 focus:ring-4 focus:ring-primary/5">
        </div>
        ...
        <div class="pt-4 border-t border-gray-100 dark:border-white/5 space-y-4">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">{{ __('file.Min Weight') }}
                        (kg)</label>
                    <input type="number" step="0.01" name="min_weight"
                        value="{{ old('min_weight', $rate ? $rate->min_weight : '') }}"
                        class="block w-full rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50/100 dark:bg-surface-tonal-a20 px-4 py-2.5 text-sm font-normal shadow-sm outline-none transition-all focus:bg-white dark:focus:bg-surface-tonal-a30 focus:border-primary/50 focus:ring-4 focus:ring-primary/5">
                </div>
                <div>
                    <label
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">{{ __('file.Max Weight') }}
                        (kg)</label>
                    <input type="number" step="0.01" name="max_weight"
                        value="{{ old('max_weight', $rate ? $rate->max_weight : '') }}"
                        class="block w-full rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50/100 dark:bg-surface-tonal-a20 px-4 py-2.5 text-sm font-normal shadow-sm outline-none transition-all focus:bg-white dark:focus:bg-surface-tonal-a30 focus:border-primary/50 focus:ring-4 focus:ring-primary/5">
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <label
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">{{ __('file.Min Price') }}
                        ({{ $currency_symbol }})</label>
                    <input type="number" step="0.01" name="min_price"
                        value="{{ old('min_price', $rate ? $rate->min_price : '') }}"
                        class="block w-full rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50/100 dark:bg-surface-tonal-a20 px-4 py-2.5 text-sm font-normal shadow-sm outline-none transition-all focus:bg-white dark:focus:bg-surface-tonal-a30 focus:border-primary/50 focus:ring-4 focus:ring-primary/5">
                </div>
                <div>
                    <label
                        class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">{{ __('file.Max Price') }}
                        ({{ $currency_symbol }})</label>
                    <input type="number" step="0.01" name="max_price"
                        value="{{ old('max_price', $rate ? $rate->max_price : '') }}"
                        class="block w-full rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50/100 dark:bg-surface-tonal-a20 px-4 py-2.5 text-sm font-normal shadow-sm outline-none transition-all focus:bg-white dark:focus:bg-surface-tonal-a30 focus:border-primary/50 focus:ring-4 focus:ring-primary/5">
                </div>
            </div>
            ...
            <div class="pt-2">
                <label
                    class="block text-sm font-medium text-emerald-600 dark:text-emerald-400 mb-1.5">{{ __('file.Free Shipping Threshold') }}
                    ({{ $currency_symbol }})</label>
                <input type="number" step="0.01" name="free_shipping_threshold"
                    value="{{ old('free_shipping_threshold', $rate ? $rate->free_shipping_threshold : '') }}"
            ...
            <label
                ...
                <div class="ml-3">
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                        {{ __('file.enable_rate_customer_note') }}</p>
                </div>
            </label>
        </div>
    </div>
    ...
    <div class="flex gap-3 pt-6 border-t border-gray-100 dark:border-white/5">
        ...
        @else
            <a href="{{ route('shipping.rates.index') }}"
                class="flex-1 px-5 py-2.5 bg-white dark:bg-surface-tonal-a20 border border-gray-200 dark:border-white/10 rounded-xl text-sm font-medium text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-surface-tonal-a30 transition-all active:scale-95 text-center">
                {{ __('file.cancel') }}
            </a>
        @endif
        <button type="submit"
            ...
            <span
                id="rate-drawer-save-text">{{ $rate ? __('file.save_changes') : __('file.save_shipping_rate') ?? 'Save Shipping Rate' }}</span>
            <div id="rate-drawer-loader"
                class="hidden w-4 h-4 border-2 border-indigo-500 border-t-transparent rounded-full animate-spin"></div>
        </button>
    </div>
</form>