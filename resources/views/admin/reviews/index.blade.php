    ...
    <div class="admin-page animate-fade-in-up">
        <div class="admin-page-inner">
            <nav class="admin-breadcrumb mt-6" aria-label="Breadcrumb">
                ...
                <span class="active">{{ __('file.product_reviews') ?? 'Product Reviews' }}</span>
            </nav>

            <div class="admin-page-header">
                <div>
                    <h1 class="admin-page-title">{{ __('file.product_reviews') ?? 'Product Reviews' }}</h1>
                    <p class="admin-page-subtitle">{{ __('file.manage_reviews') ?? 'Moderate and curate customer feedback and sentiment' }}</p>
                </div>
            </div>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                const table = $('#application-table').DataTable({
                    ...
                    language: {
                        search: "",
                        searchPlaceholder: "{{ __('file.search_reviews') ?? "Search Reviews..." }}",
                        lengthMenu: "{{ __('file.dt_length_menu') ?? "_MENU_" }}",
                        info: "{{ __('file.showing_reviews') ?? "Showing _START_ to _END_ of _TOTAL_ Reviews" }}",
                        infoEmpty: "{{ __('file.no_reviews_found') ?? "No reviews found" }}",
                        emptyTable: "{{ __('file.no_reviews_found') ?? "No reviews found" }}",
                        processing: false,
                    },
                    autoWidth: false,
                    scrollX: false
