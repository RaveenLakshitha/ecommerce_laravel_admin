...
@section('title', __('file.age_groups'))
...
@section('content')
        ...
        <div class=" flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    {{ __('file.manage_age_group_records') }}
                </p>
            </div>
            <a href="{{ route('age-groups.create') }}"
                ...
                {{ __('file.add_age_group') }}
            </a>
        </div>

        <div id="bulk-delete-form" class="hidden mb-6">
            <form method="POST" action="{{ route('age-groups.bulkDelete') }}" id="bulk-delete-form-el"
                ...
                <span class="text-sm font-medium text-red-800 dark:text-red-300">
                    <span id="selected-count">0</span> {{ __('file.age_groups_selected') }}
                </span>
                <button type="submit"
        ...
        <div id="profile-panel"
            ...
            <div class="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-gray-700">
                <div>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('file.age_group_details') }}</p>
                </div>
            ...
            <div class="flex-1 overflow-y-auto px-5 py-5 text-sm space-y-6">
                <div>
                    ...
                    <div class="text-gray-900 dark:text-white font-medium" id="drawer-age-range"></div>
                </div>
                <div>
                    <h4 class="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-2">
                        {{ __('file.doctors') }}
                ...
                <div>
                    ...
                    <div id="drawer-status" class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium">
                    </div>
                </div>
            </div>
    ...
    <div id="edit-drawer" class="fixed inset-0 z-50 hidden overflow-hidden">
        ...
        <div id="edit-panel"
            ...
            <div class="flex items-center justify-between px-5 py-4 border-b border-gray-200 dark:border-gray-700">
                <div>
                    <h3 class="text-lg font-semibold text-gray-900 dark:text-white">{{ __('file.edit_age_group') }}</h3>
                    <p class="text-xs text-gray-500 dark:text-gray-400">{{ __('file.age_group_details') }}</p>
                </div>
            ...
            <form id="edit-form" class="flex-1 flex flex-col">
                ...
                <div class="flex-1 overflow-y-auto px-5 py-5 text-sm space-y-6">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label
                                class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('file.min_age') }}</label>
                            <input type="number" name="min_age" id="edit-min_age" min="0"
                                class="w-full px-2 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-500 focus:border-transparent dark:bg-transparent dark:text-white transition-shadow" />
                        </div>
                        <div>
                            <label
                                class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-2">{{ __('file.max_age') }}</label>
                            <input type="number" name="max_age" id="edit-max_age" min="0"
                                class="w-full px-2 py-2 text-sm border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-500 focus:border-transparent dark:bg-transparent dark:text-white transition-shadow" />
                        </div>
                    </div>
                    <div>
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="is_active" id="edit-is_active" value="1"
                            class="w-4 h-4 rounded border-gray-300 text-indigo-600 focus:ring-indigo-500">
                        <label for="edit-is_active" class="ml-2 text-sm text-gray-700 dark:text-gray-300">{{ __('file.active') }}</label>
                    </div>
                </div>
                <div class="px-5 py-4 bg-gray-50 dark:bg-gray-900/50 border-t border-gray-200 dark:border-gray-700">
                    <button type="submit"
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                const table = $('#docapp-table').DataTable({
                    ...
                    serverSide: true,
                    responsive: false,
                    ajax: '{{ route('age-groups.datatable') }}',
                    order: [[1, 'asc']],
                    columnDefs: [
                    ...
                    columns: [
                        ...
                        { data: 'age_range', className: 'text-center font-medium' },
                        { data: 'doctors_count', className: 'text-center font-medium' },
                        {
                            data: 'description',
                            render: data => data ? `<div class="max-w-md truncate" title="${data}">${data}</div>` : '<span class="text-gray-400">—</span>'
                        },
                        {
                            data: null,
                            orderable: false,
                            ...
                            buttons: [
                                ...
                                },
                                {
                                    ...
                                    buttons: [
                                        ...
                                        { extend: 'excel', text: 'Excel', filename: 'Age Group List{{ date("Y-m-d") }}', exportOptions: { columns: [0, 1, 2, 3, 4] } },
                                        { extend: 'csv', text: 'CSV', filename: 'Age Group List{{ date("Y-m-d") }}', exportOptions: { columns: [0, 1, 2, 3, 4] } },
                                        { extend: 'pdf', text: 'PDF', filename: 'Age Group List{{ date("Y-m-d") }}', title: 'Age Group List', exportOptions: { columns: [0, 1, 2, 3, 4] } },
                    ...
                    language: {
                        search: "",
                        searchPlaceholder: "{{ __('file.search_age_groups') }}",
                        lengthMenu: "{{ __('file.show_entries') }}",
                        info: "{{ __('file.showing_entries') }}",
                        emptyTable: "{{ __('file.no_age_groups_found') }}",
                ...
                window.confirmDelete = function (url) {
                    if (!confirm('{{ __("file.confirm_delete_age_group") }}')) return;
                    ...
                    $.ajax({
                        ...
                        error: function (xhr) {
                            const msg = xhr.responseJSON?.message || 'Error deleting age group.';
                ...
                $('#bulk-delete-form-el').on('submit', function (e) {
                    ...
                    $.ajax({
                        ...
                        success: (response) => {
                            ...
                            updateBulkDeleteUI();
                            if (response.success) {
                                if (typeof showNotification === 'function') showNotification('Success', response.message || 'Age groups deleted successfully.', 'success');
                            } else {
                                if (typeof showNotification === 'function') showNotification('Error', response.message, 'error');
                            }
                        },
                ...
                window.openProfileDrawer = function (group) {
                    document.getElementById('drawer-name').textContent = group.name;
                    document.getElementById('drawer-age-range').textContent = group.age_range || '—';
                    document.getElementById('drawer-doctors').textContent = group.doctors_count;
                    document.getElementById('drawer-description').textContent = group.description || '—';

                    const statusEl = document.getElementById('drawer-status');
                    if (group.is_active) {
                        statusEl.textContent = '{{ __('file.active') }}';
                        statusEl.className = 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-300';
                    } else {
                        statusEl.textContent = '{{ __('file.inactive') }}';
                        statusEl.className = 'inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-gray-100 text-gray-800 dark:bg-gray-700 dark:text-gray-300';
                    }
                ...
                window.openEditDrawer = function (group) {
                    document.getElementById('edit-id').value = group.id;
                    document.getElementById('edit-name').value = group.name || '';
                    document.getElementById('edit-min_age').value = group.min_age ?? '';
                    document.getElementById('edit-max_age').value = group.max_age ?? '';
                    document.getElementById('edit-description').value = group.description || '';
                    document.getElementById('edit-is_active').checked = !!group.is_active;
                ...
                document.getElementById('edit-form').addEventListener('submit', function (e) {
                    ...
                    fetch(`{{ url('age-groups') }}/${id}`, {
                        ...
                        .catch(error => {
                            console.error(error);
                            if (typeof showNotification === 'function') showNotification('Error', 'Failed to update: ' + error.message, 'error');
                        });
                });
