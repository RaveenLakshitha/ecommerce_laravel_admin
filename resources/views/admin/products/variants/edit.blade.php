    ...
    <div class="admin-page-inner">
        ...
        <div class="admin-page-header">
            <div>
                ...
                <h1 class="admin-page-title">Edit Variant: {{ $variant->sku }}</h1>
                <p class="admin-page-subtitle">Updating variant for product <span class="font-semibold text-gray-900 dark:text-white">{{ $product->name }}</span></p>
            </div>
        </div>
        ...
        <form action="{{ route('products.variants.update', [$product->id, $variant->id]) }}" method="POST" class="space-y-6">
                ...
                <div class="admin-card-body">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="text" name="sku" id="sku" value="{{ old('sku', $variant->sku) }}" required class="fi">
                        </div>
                        <div>
                            <label for="barcode" class="fi-label">Barcode</label>
                            <input type="text" name="barcode" id="barcode" value="{{ old('barcode', $variant->barcode) }}" class="fi">
                        </div>
                        <div>
                            <label for="price" class="fi-label fi-label-required">Price</label>
                            <input type="number" step="0.01" name="price" id="price" value="{{ old('price', $variant->price) }}" required class="fi">
                        </div>
                        <div>
                            <label for="sale_price" class="fi-label">Sale Price</label>
                            <input type="number" step="0.01" name="sale_price" id="sale_price" value="{{ old('sale_price', $variant->sale_price) }}" class="fi">
                        </div>
                        <div>
                            <label for="stock_quantity" class="fi-label fi-label-required">Stock Quantity</label>
                            <input type="number" name="stock_quantity" id="stock_quantity" value="{{ old('stock_quantity', $variant->stock_quantity) }}" required class="fi">
                        </div>
                        <div>
                            <label for="low_stock_threshold" class="fi-label">Low Stock Threshold</label>
                            <input type="number" name="low_stock_threshold" id="low_stock_threshold" value="{{ old('low_stock_threshold', $variant->low_stock_threshold) }}" class="fi">
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="admin-card">
                ...
                <div class="admin-card-body">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        <div>
                            ...
                            <input type="number" name="weight_grams" id="weight_grams" value="{{ old('weight_grams', $variant->weight_grams) }}" class="fi">
                        </div>
                        <div>
                            <label for="dimensions" class="fi-label">Dimensions (L x W x H)</label>
                            <input type="text" name="dimensions" id="dimensions" value="{{ old('dimensions', $variant->dimensions) }}" class="fi">
                        </div>
                ...
                <div class="px-6 py-4 border-t border-gray-100 dark:border-surface-tonal-a30 flex justify-end gap-3">
                    ...
                    <button type="submit" class="admin-btn-primary !w-auto">Update Variant</button>
                </div>
            </div>
        </form>
    </div>
