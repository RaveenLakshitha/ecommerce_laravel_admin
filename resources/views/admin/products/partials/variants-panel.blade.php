                            ...
                            <th class="px-3 py-2.5">
                                <div class="flex items-center gap-2">
                                    <input type="checkbox" id="vp-select-all" onchange="VariantsPanel.toggleAll(this)"
                                        class="h-4 w-4 rounded border-gray-300 dark:border-white/10 text-primary dark:text-primary focus:ring-primary/20 bg-white dark:bg-surface-tonal-a20">
                                    <span
                                        class="text-sm font-medium text-gray-500 dark:text-gray-400">{{ __('file.variant') }}</span>
                                </div>
                            </th>
                            <th class="px-3 py-2.5 text-sm font-medium text-gray-500 dark:text-gray-400">
                                {{ __('file.price') }}</th>
                            <th class="px-3 py-2.5 text-sm font-medium text-gray-500 dark:text-gray-400">
                                {{ __('file.sku') }}</th>
                            <th class="px-3 py-2.5 text-sm font-medium text-gray-500 dark:text-gray-400">
                                {{ __('file.inventory') }}</th>
                            <th class="px-3 py-2.5 text-sm font-medium text-gray-500 dark:text-gray-400">
                                {{ __('file.image') }}</th>
                        </tr>
                    </thead>
        ...
        const VariantsPanel = {
                ...
                this.options.forEach((opt, optIndex) => {
                    ...
                    <div class="flex items-stretch gap-3">
                        <div class="w-40 flex-shrink-0">
                            <input type="text"
                                list="${datalistId}"
                                class="vp-opt-name block w-full h-full rounded-xl border border-gray-200 dark:border-white/5 bg-gray-50 dark:bg-surface-tonal-a10 px-4 py-2.5 text-sm font-normal text-gray-900 dark:text-white outline-none focus:bg-white dark:focus:bg-surface-tonal-a20 focus:border-primary/50 focus:ring-4 focus:ring-primary/5 transition-all"
                                placeholder="{{ __('file.option_name_placeholder') }}"
                                value="${this.esc(opt.name)}"
                                oninput="VariantsPanel.updateOptionName(${optIndex}, this.value)">
                        </div>
                        <div class="flex-1 flex flex-wrap items-center gap-1.5 min-h-[44px] rounded-xl border border-gray-200 dark:border-white/5 bg-gray-50 dark:bg-surface-tonal-a10 px-3 py-2 cursor-text focus-within:bg-white dark:focus-within:bg-surface-tonal-a20 focus-within:border-primary/50 focus-within:ring-4 focus-within:ring-primary/5 transition-all"
                            ...
                            <input
                                ...
                                placeholder="${opt.values.length === 0 ? "{{ __('file.add_values_and_press_enter') }}" : ''}"
                                class="flex-1 min-w-[120px] bg-transparent outline-none text-sm font-normal text-gray-700 dark:text-white placeholder:text-gray-400"
            ...
            renderValueSuggestions(optIndex) {
                ...
                suggestionsContainer.innerHTML = remainingValues.map(v =>
                    ...
                    class="px-2.5 py-1 rounded-lg border border-gray-200 dark:border-surface-tonal-a30 text-xs text-gray-600 dark:text-gray-400 hover:bg-primary/5 hover:border-primary/30 transition-all font-medium">
                    + ${this.esc(v)}
                </button>`
                ).join('');
            },
            ...
            renderVariantsTable() {
                ...
                combos.forEach((combo, rowIndex) => {
                    ...
                    tr.className = 'hover:bg-gray-50 dark:hover:bg-surface-tonal-a30 transition-colors';
                    tr.dataset.key = key;
                    tr.innerHTML = `
                    <td class="px-3 py-2.5">
                        <div class="flex items-center gap-2.5">
                            <input type="checkbox" name="variants[${rowIndex}][enabled]" value="1"
                                ...
                                class="h-4 w-4 rounded border-gray-300 dark:border-white/10 text-primary dark:text-primary focus:ring-primary/20 bg-white dark:bg-surface-tonal-a20 cursor-pointer">
                            ${id ? `<input type="hidden" name="variants[${rowIndex}][id]" value="${id}">` : ''}
                            ${optHiddens}
                            <span class="text-sm font-medium leading-tight">${labelHtml}</span>
                        </div>
                    </td>
                    <td class="px-3 py-2.5">
                        <div class="relative">
                    ...
                    <td class="px-3 py-2.5">
                        <input type="text" name="variants[${rowIndex}][sku]" value="${sku}"
                            placeholder="SKU-001"
                            class="w-32 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-surface-tonal-a30 px-3 py-2 text-sm font-normal text-gray-900 dark:text-white outline-none focus:border-primary/50 focus:ring-4 focus:ring-primary/5 transition-all">
                    </td>
                    <td class="px-3 py-2.5">
                        <input type="number" name="variants[${rowIndex}][stock_quantity]"
                            value="${qty}" min="0"
                            class="w-24 rounded-xl border border-gray-200 dark:border-white/10 bg-white dark:bg-surface-tonal-a30 px-3 py-2 text-sm font-normal text-gray-900 dark:text-white outline-none focus:border-primary/50 focus:ring-4 focus:ring-primary/5 transition-all">
                    </td>
                    <td class="px-3 py-2.5">
                        <div class="relative group/img">
                            ...
                            <div id="vp-img-${rowIndex}"
                                ...
                                title="{{ __('file.click_to_upload_variant_image') }}"
