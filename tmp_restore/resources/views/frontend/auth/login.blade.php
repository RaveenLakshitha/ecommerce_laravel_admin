<x-ecommerce-guest-layout>
    ...
    <div class="lg:hidden flex flex-col items-center mb-8 text-center pt-8">
        <div class="w-16 h-16 bg-indigo-100 rounded-2xl flex items-center justify-center p-3 shadow-md mb-4 text-indigo-600">
            @if($store_logo ?? false)
                <img src="{{ $store_logo }}" alt="{{ $store_name ?? __('messages.Ecommerce Store') }} Logo" class="w-12 h-12 object-contain">
            @else
        ...
        <h1 class="text-2xl font-bold text-slate-800">
            {{ $store_name ?? __('messages.Ecommerce Store') }}
        </h1>
    </div>
    ...
    <div class="mb-10 lg:text-left text-center">
        ...
        <p class="text-slate-500 text-base">{{ __('messages.Please enter your credentials to access your account.') }}</p>
    </div>
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-6">
        ...
        <div class="pt-2">
            <button type="submit" class="w-full flex justify-center items-center py-3.5 px-4 text-base font-semibold text-white rounded-xl bg-sky-600 hover:bg-sky-500 focus:ring-4 focus:ring-sky-500/30 transition-all duration-200 shadow-lg shadow-sky-600/20 active:scale-[0.98]">
                <span>{{ __('messages.Sign in to your account') }}</span>
