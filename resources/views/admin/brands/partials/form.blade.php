<form id="brand-drawer-form" action="{{ $brand ? route('brands.update', $brand->id) : route('brands.store') }}"
    ...
    <div class="space-y-4">
        ...
        <div>
            ...
            <input type="url" name="website_url" id="drawer_website_url"
                value="{{ $brand ? $brand->website_url : old('website_url') }}" placeholder="https://example.com"
                class="block w-full rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50/100 dark:bg-surface-tonal-a20 px-4 py-2.5 text-sm font-normal shadow-sm placeholder:text-gray-400 dark:placeholder:text-gray-500 text-gray-900 dark:text-white outline-none transition-all focus:bg-white dark:focus:bg-surface-tonal-a30 focus:border-primary/50 focus:ring-4 focus:ring-primary/5">
        </div>
        ...
        <div>
            ...
            <input type="number" name="sort_order" id="drawer_sort_order"
                value="{{ $brand ? $brand->sort_order : old('sort_order', 0) }}"
                class="block w-full rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50/100 dark:bg-surface-tonal-a20 px-4 py-2.5 text-sm font-normal shadow-sm placeholder:text-gray-400 dark:placeholder:text-gray-500 text-gray-900 dark:text-white outline-none transition-all focus:bg-white dark:focus:bg-surface-tonal-a30 focus:border-primary/50 focus:ring-4 focus:ring-primary/5">
        </div>
        ...
        <div>
            <label
                ...
                <div class="ml-3">
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{{ __('file.highlight_on_storefront') }}
                    </p>
                </div>
            </label>
        </div>
        ...
        <div>
            ...
            <div class="relative w-full rounded-xl overflow-hidden border-2 border-dashed border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-surface-tonal-a10 cursor-pointer group/upload hover:border-primary transition-all"
                ...
                <div id="drawer-logo-hover"
                    ...
                    <span
                        class="text-xs font-medium text-white uppercase tracking-widest">{{ __('file.change_logo') }}</span>
                </div>

                <input type="file" name="logo" id="drawer-logo-input" class="hidden" accept="image/*"
                    onchange="previewDrawerLogo(this)">
            </div>
        ...
        <div class="pt-4 border-t border-gray-100 dark:border-white/5 space-y-4">
            ...
            <div>
                ...
                <input type="text" name="meta_title" id="drawer_meta_title"
                    value="{{ $brand ? $brand->meta_title : old('meta_title') }}"
                    placeholder="{{ __('file.seo_title_search') }}"
                    class="block w-full rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50/100 dark:bg-surface-tonal-a20 px-4 py-2.5 text-sm font-normal shadow-sm outline-none transition-all focus:bg-white dark:focus:bg-surface-tonal-a30">
            </div>
            <div>
                ...
                <textarea name="meta_description" id="drawer_meta_description" rows="2"
                    ...
                    class="block w-full rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50/100 dark:bg-surface-tonal-a20 px-4 py-2.5 text-sm font-normal shadow-sm outline-none transition-all focus:bg-white dark:focus:bg-surface-tonal-a30 resize-y">{{ $brand ? $brand->meta_description : old('meta_description') }}</textarea>
            </div>
        </div>
    </div>
