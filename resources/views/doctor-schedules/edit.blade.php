...
@section('title', __('file.edit_doctor_schedule'))
    ...
    <div class="px-4 sm:px-6 lg:px-4 pb-4 sm:py-12 pt-20">
        <div class=" mb-8">
            <div class="flex items-center text-sm text-gray-500 dark:text-gray-400 mb-3">
                ...
                <span class="text-gray-900 dark:text-white">{{ __('file.edit_schedule') }}</span>
            </div>
            <h1 class="text-3xl font-semibold text-gray-900 dark:text-white">{{ __('file.edit_schedule') }}</h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">{{ __('file.update_recurring_schedule') }}</p>
        </div>

        <form method="POST" action="{{ route('doctor-schedules.update', $doctorSchedule) }}" class="space-y-8">
            @csrf
            @method('PUT')
                        ...
                        <div>
                            ...
                            <select name="room_id" required
                                ...
                                @foreach($rooms as $room)
                                    <option value="{{ $room->id }}" {{ old('room_id', $doctorSchedule->room_id) == $room->id ? 'selected' : '' }}>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            ...
                            <input type="time" name="start_time"
                                value="{{ old('start_time', $doctorSchedule->start_time->format('H:i')) }}" required class="w-full px-3 py-2.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg 
                                                  focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-500 focus:border-transparent 
                                                  dark:bg-transparent dark:text-white transition-shadow">
                            @error('start_time')
                                <p class="mt-1.5 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="time" name="end_time"
                                value="{{ old('end_time', $doctorSchedule->end_time->format('H:i')) }}" required class="w-full px-3 py-2.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg 
                                                  focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-500 focus:border-transparent 
                                                  dark:bg-transparent dark:text-white transition-shadow">
                            @error('end_time')
                                <p class="mt-1.5 text-xs text-red-600 dark:text-red-400">{{ $message }}</p>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-4">
                            ...
                            @foreach($days as $day)
                                <label class="flex items-center space-x-3 cursor-pointer">
                                    <input type="checkbox" name="days_of_week[]" value="{{ $day }}" {{ in_array($day, $currentDays) ? 'checked' : '' }}
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            <div>
                                ...
                                <input type="date" name="valid_from"
                                    value="{{ old('valid_from', $doctorSchedule->valid_from?->format('Y-m-d')) }}"
                                    class="w-full px-3 py-2.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg 
                                                      focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-500 focus:border-transparent 
                            ...
                            <div>
                                ...
                                <input type="date" name="valid_until"
                                    value="{{ old('valid_until', $doctorSchedule->valid_until?->format('Y-m-d')) }}"
                                    class="w-full px-3 py-2.5 text-sm border border-gray-300 dark:border-gray-600 rounded-lg 
                                                      focus:ring-2 focus:ring-gray-900 dark:focus:ring-gray-500 focus:border-transparent 
                                                      dark:bg-transparent dark:text-white [color-scheme:light] dark:[color-scheme:dark] transition-shadow">
                                @error('valid_until')
