            ...
            <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-8">
                <div>
                    <div class="flex items-center gap-4">
                        <h1 class="text-2xl font-bold tracking-tight text-gray-900 dark:text-white">{{ $product->name }}
                        </h1>
                        <div class="flex gap-2">
                            @if($product->is_visible)
                                <span
                                    class="px-2.5 py-0.5 rounded-md text-[10px] font-black bg-emerald-100 text-emerald-700 dark:bg-emerald-500/10 dark:text-emerald-400 uppercase tracking-widest border border-emerald-200 dark:border-emerald-500/20">Visible</span>
                            @else
                                <span
                                    class="px-2.5 py-0.5 rounded-md text-[10px] font-black bg-gray-100 text-gray-600 dark:bg-surface-tonal-a30 dark:text-gray-400 uppercase tracking-widest border border-gray-200 dark:border-surface-tonal-a30">Hidden</span>
                            @endif
                            @if($product->is_featured)
                                <span
                                    class="px-2.5 py-0.5 rounded-md text-[10px] font-black bg-amber-100 text-amber-700 dark:bg-amber-500/10 dark:text-amber-400 uppercase tracking-widest border border-amber-200 dark:border-amber-500/20">Featured</span>
                            @endif
                        </div>
                    </div>
                ...
                <div class="flex flex-wrap gap-3">
                    ...
                    <form action="{{ route('products.destroy', $product->id) }}" method="POST"
                        ...
                        @method('DELETE')
                        <button type="submit"
                            class="px-5 py-2.5 bg-red-50 dark:bg-red-500/10 border border-red-100 dark:border-red-500/20 rounded-lg text-sm font-semibold text-red-600 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-500/20 transition-all shadow-sm active:scale-[0.98]">
                            Delete
                        </button>
                    </form>
                </div>
                ...
                <div class="lg:col-span-2 space-y-4">
                        ...
                        <div class="p-4">
                            <div class="grid grid-cols-1 sm:grid-cols-3 gap-y-6 gap-x-8">
                                ...
                                <div class="space-y-1">
                                    ...
                                    <p
                                        class="text-xl font-bold {{ $product->sale_price ? 'text-emerald-500' : 'text-gray-900 dark:text-white' }}">
                                        {{ $product->sale_price ? '$' . number_format($product->sale_price, 2) : 'None' }}
                                    </p>
                                </div>
                                <div class="space-y-1">
                                ...
                                <div class="sm:col-span-2 space-y-1">
                                    ...
                                    <div class="flex flex-wrap gap-1 mt-1">
                                        @forelse($product->categories as $category)
                                            <span
                                                class="px-2 py-0.5 rounded-md bg-gray-100 dark:bg-surface-tonal-a30 text-gray-600 dark:text-gray-400 text-[10px] font-bold">{{ $category->name }}</span>
                                        @empty
                        ...
                        <div class="p-4 space-y-6">
                            @if($product->short_description)
                                <div class="space-y-1.5">
                                    <h3 class="text-[10px] font-black text-black dark:text-white uppercase tracking-widest">
                            ...
                            <div class="space-y-1.5">
                                <h3 class="text-[10px] font-black text-black dark:text-white uppercase tracking-widest">Full
                                    Story</h3>
                                @if($product->description)
                                    <div
                                        class="prose prose-indigo dark:prose-invert max-w-none text-sm text-gray-600 dark:text-gray-300">
                    ...
                    <div
                        ...
                        <div
                            ...
                            <span
                                class="text-[10px] font-bold text-indigo-500 uppercase tracking-widest">{{ $product->variants->count() }}
                                Variants</span>
                        </div>
                            ...
                            <table class="w-full text-left border-collapse">
                                ...
                                <tbody class="divide-y divide-gray-50 dark:divide-surface-tonal-a30">
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="4"
                                                class="px-6 py-10 text-center text-xs font-medium text-gray-500 uppercase tracking-widest">
                ...
                <div class="col-span-1 space-y-4">
                    ...
                    <div
                        ...
                        <div class="p-4 space-y-4">
                            <div class="space-y-1">
                                ...
                                <p class="text-sm font-bold text-gray-700 dark:text-white">
                                    {{ $product->meta_title ?: 'None' }}</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Meta Description
                                </p>
                                <p class="text-sm text-gray-600 dark:text-gray-300">
                                    {{ $product->meta_description ?: 'None' }}</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Meta Keywords</p>
                                <p class="text-sm font-bold text-gray-700 dark:text-white">
                                    {{ $product->meta_keywords ?: 'None' }}</p>
                            </div>
                            <div class="space-y-1">
                                <p class="text-[10px] font-black text-gray-400 uppercase tracking-widest">Canonical URL</p>
                                @if($product->canonical_url)
                                    <a href="{{ $product->canonical_url }}" target="_blank"
                                        class="text-sm font-bold text-indigo-600 dark:text-indigo-400 hover:underline truncate block">{{ $product->canonical_url }}</a>
                                @else
                    ...
                    <div
                        ...
                        <div class="p-4">
                            ...
                            @if($primary)
                                <div
                                    class="aspect-square rounded-xl border border-gray-100 dark:border-surface-tonal-a30 overflow-hidden mb-3 bg-gray-50 dark:bg-surface-tonal-a30/20 relative">
                                ...
                                <div class="grid grid-cols-4 gap-2">
                                    @foreach($product->images as $image)
                                        @if($image->id !== $primary->id)
                                            <div
                                                class="aspect-square rounded-lg border border-gray-100 dark:border-surface-tonal-a30 overflow-hidden bg-gray-50 dark:bg-surface-tonal-a30/20">
                            ...
                            @else
                                <div
                                    class="aspect-square rounded-xl border-2 border-dashed border-gray-100 dark:border-surface-tonal-a30 flex flex-col items-center justify-center gap-2 text-gray-400 p-8 text-center bg-gray-100/50 dark:bg-surface-tonal-a30/20">
                    ...
                    <div
                        class="bg-indigo-600 rounded-lg shadow-sm border border-indigo-500 overflow-hidden relative group p-4 text-white hover:bg-indigo-700 transition-colors">
                        <div
                        ...
                        <div class="space-y-3 relative z-10">
                            ...
                            <div class="w-full bg-white/20 h-1 rounded-full overflow-hidden">
                                <div class="bg-white h-full transition-all duration-1000"
                                    style="width: {{ $product->variants->sum('available_quantity') > 0 ? '100%' : '0%' }}">
                                </div>
                            </div>
