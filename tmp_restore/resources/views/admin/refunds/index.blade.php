...
@section('title', __('file.refunds') ?? 'Refunds Management')
...
<div class="px-4 sm:px-6 lg:px-8 pb-4 sm:py-12 pt-20">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h1 class="text-2xl sm:text-3xl font-semibold text-gray-900 dark:text-primary-a0">
                {{ __('file.refunds') ?? 'Refunds Management' }}
            </h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {{ __('file.manage_refunds') ?? 'Monitor fiscal reversals and coordinate restitution flows' }}
            </p>
        </div>
    </div>
    ...
    <div id="bulk-delete-form" class="hidden mb-6">
        <form method="POST" action="{{ route('refunds.bulkDelete') }}" id="bulk-delete-form-el"
            ...
            <table id="application-table" class="w-full divide-y divide-gray-200 dark:divide-surface-tonal-a30 nowrap" style="width:100%">
                <thead class="bg-gray-50 dark:bg-surface-tonal-a10 border-b border-gray-200 dark:border-surface-tonal-a30">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider all">Refund ID</th>
                        <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider desktop">Order</th>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        const table = $('#application-table').DataTable({
            ...
            columns: [
                ...
                { 
                    data: 'refund_id_html', name: 'id',
                    render: data => `<span class="text-sm font-medium text-gray-900 dark:text-white font-mono uppercase">${data}</span>`
                },
                { 
                ...
                { 
                    ...
                    render: data => `<span class="text-sm font-bold text-rose-600 dark:text-rose-400 italic">-${data}</span>`
                },
                {
                    data: null,
                    orderable: false,
