<form action="{{ $zone ? route('shipping.zones.update', $zone->id) : route('shipping.zones.store') }}" method="POST"
    id="zone-drawer-form" class="space-y-6">
    @csrf
    @if($zone)
        @method('PUT')
    @endif
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                ...
                <input type="text" name="country_code"
                    value="{{ old('country_code', $zone ? $zone->country_code : '') }}" required
    ...
    <div class="flex gap-3 pt-6 border-t border-gray-100 dark:border-white/5">
        ...
        <button type="submit"
            ...
            <span
                id="zone-drawer-save-text">{{ $zone ? __('file.save_changes') : __('file.save_shipping_zone') ?? 'Save Shipping Zone' }}</span>
            <div id="zone-drawer-loader"
