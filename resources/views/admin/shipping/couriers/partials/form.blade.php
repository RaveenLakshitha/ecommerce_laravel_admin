<form action="{{ $courier ? route('shipping.couriers.update', $courier->id) : route('shipping.couriers.store') }}"
    method="POST" id="courier-drawer-form" class="space-y-6">
    @csrf
    @if($courier)
        @method('PUT')
    @endif
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                    ...
                    <input type="password" name="api_secret"
                        value="{{ old('api_secret', $courier ? $courier->api_secret : '') }}"
                        class="block w-full rounded-xl border border-gray-200 dark:border-white/10 bg-gray-50/100 dark:bg-surface-tonal-a20 px-4 py-2.5 text-sm font-normal shadow-sm outline-none transition-all focus:bg-white dark:focus:bg-surface-tonal-a30 focus:border-primary/50 focus:ring-4 focus:ring-primary/5">
                </div>
            </div>
        </div>
    ...
    <div class="flex gap-3 pt-6 border-t border-gray-100 dark:border-white/5">
        ...
        <button type="submit"
            ...
            <span
                id="courier-drawer-save-text">{{ $courier ? __('file.save_changes') : __('file.save_provider') ?? 'Save Courier Provider' }}</span>
            <div id="courier-drawer-loader"
