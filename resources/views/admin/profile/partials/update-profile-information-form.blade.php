<section class="space-y-10">
    <header>
        <h2 class="text-[10px] font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em] mb-2">Identification Profile</h2>
        <p class="text-sm font-black text-gray-900 dark:text-white uppercase tracking-tighter italic">Update your account's profile information and primary communication endpoint.</p>
    </header>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="space-y-8">
        ...
        <div class="space-y-3 p-6 bg-gray-50/50 dark:bg-surface-tonal-a10/50 rounded-3xl border border-gray-100 dark:border-surface-tonal-a30 shadow-inner">
            <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest">{{ __('Name') }} <span class="text-rose-500">*</span></label>
            ...
            <x-input-error class="text-[10px] font-black text-rose-500 uppercase tracking-widest mt-1 italic" :messages="$errors->get('name')" />
        </div>

        <div class="space-y-3 p-6 bg-gray-50/50 dark:bg-surface-tonal-a10/50 rounded-3xl border border-gray-100 dark:border-surface-tonal-a30 shadow-inner">
            <label class="block text-[10px] font-black text-gray-400 uppercase tracking-widest">{{ __('Email') }} <span class="text-rose-500">*</span></label>
            ...
            <x-input-error class="text-[10px] font-black text-rose-500 uppercase tracking-widest mt-1 italic" :messages="$errors->get('email')" />

            @if ($user instanceof \Illuminate\Contracts\Auth\MustVerifyEmail && ! $user->hasVerifiedEmail())
                <div class="mt-4 p-4 bg-amber-50 dark:bg-amber-500/10 rounded-2xl border border-amber-100 dark:border-amber-500/20">
                    <p class="text-[10px] font-black text-amber-800 dark:text-amber-400 uppercase tracking-widest leading-relaxed italic">
                        {{ __('Your email address is unverified.') }}
                        <button form="send-verification" class="ml-2 underline text-amber-600 dark:text-amber-300 hover:text-amber-700">
