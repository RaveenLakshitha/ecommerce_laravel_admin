        ...
        <div class="admin-page-inner">
            ...
            <div class="admin-page-header">
                <div>
                    ...
                    <h1 class="admin-page-title">Edit Collection</h1>
                    <p class="admin-page-subtitle">Modify properties for: <span class="font-semibold text-gray-900 dark:text-white">{{ $collection->name }}</span></p>
                </div>
            </div>
                    ...
                    <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="admin-card">
                            ...
                            <div class="admin-card-body space-y-5">
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                    <div>
                                        <label for="name" class="fi-label fi-label-required">Name</label>
                                        <input type="text" name="name" id="name" value="{{ old('name', $collection->name) }}" required class="fi">
                                        @error('name') <p class="fi-error">{{ $message }}</p> @enderror
                                    </div>
                                    <div>
                                        <label for="slug" class="fi-label fi-label-required">Slug</label>
                                        <input type="text" name="slug" id="slug" value="{{ old('slug', $collection->slug) }}" required class="fi">
                                ...
                                <div>
                                    ...
                                    <textarea name="description" id="description" rows="4" placeholder="Brief description…" class="fi">{{ old('description', $collection->description) }}</textarea>
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-5 pt-1">
                                    <div>
                                        ...
                                        <input type="date" name="start_date" id="start_date"
                                            value="{{ old('start_date', $collection->start_date ? $collection->start_date->format('Y-m-d') : '') }}" class="fi">
                                        @error('start_date') <p class="fi-error">{{ $message }}</p> @enderror
                                    </div>
                                    <div>
                                        <label for="end_date" class="fi-label">End Date</label>
                                        <input type="date" name="end_date" id="end_date"
                                            value="{{ old('end_date', $collection->end_date ? $collection->end_date->format('Y-m-d') : '') }}" class="fi">
                        ...
                        <div class="admin-card">
                            ...
                            <div class="admin-card-body">
                                @php
                                    $selectedProducts = old('products', $collection->products->pluck('id')->toArray());
                                @endphp
                                <div class="space-y-4" x-data="{
                                    search: '',
                                    page: 1,
                                    perPage: 10,
                                    products: [ @foreach($products as $p) { id: {{ $p->id }}, name: '{{ addslashes($p->name) }}', checked: {{ (is_array($selectedProducts) && in_array($p->id, $selectedProducts)) ? 'true' : 'false' }} }, @endforeach ],
                                    get filtered() {
                                    ...
                                    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                                        <label class="fi-label !mb-0">Selected Products</label>
                                    ...
                                    <div class="rounded-xl border border-gray-200 dark:border-surface-tonal-a30 divide-y divide-gray-50 dark:divide-surface-tonal-a30 overflow-hidden bg-white dark:bg-surface-tonal-a20">
                                        <template x-for="product in paginated" :key="product.id">
                                            <label class="flex items-center gap-4 px-4 py-3 hover:bg-gray-50/50 dark:hover:bg-surface-tonal-a30/30 cursor-pointer transition">
                                                <input type="checkbox" name="products[]" :value="product.id" x-model="product.checked"
                                                    class="h-4 w-4 rounded border-gray-300 dark:border-surface-tonal-a30 text-gray-900 focus:ring-gray-500 transition-all">
                                                <span class="text-sm font-medium text-gray-700 dark:text-gray-300" x-text="product.name"></span>
                                            </label>
                                        </template>
                                        <div x-show="filtered.length === 0" class="p-8 text-center text-sm text-gray-400 dark:text-gray-500">No products found.</div>
                                    </div>
                                    <div class="flex items-center justify-between px-1" x-show="totalPages > 1">
                                        <span class="text-xs text-gray-400 dark:text-gray-500">Page <span x-text="page"></span> of <span x-text="totalPages"></span></span>
                                    ...
                                    <template x-for="item in products.filter(p => p.checked && !paginated.find(pg => pg.id === p.id))" :key="'hidden-'+item.id">
                            ...
                            <div class="admin-card-body space-y-6">
                                <div class="space-y-3">
                                    <label class="ck-card">
                                        <input type="checkbox" name="is_active" id="is_active" value="1" {{ old('is_active', $collection->is_active) ? 'checked' : '' }}
                                            class="h-4 w-4 rounded border-gray-300 dark:border-surface-tonal-a30 text-gray-900 focus:ring-gray-500 transition-all">
                                        <div class="ml-3">
                                            <h3 class="text-sm font-semibold text-gray-900 dark:text-white">Collection Active</h3>
                                    ...
                                    <label class="ck-card">
                                        <input type="checkbox" name="is_featured" id="is_featured" value="1" {{ old('is_featured', $collection->is_featured) ? 'checked' : '' }}
                                            class="h-4 w-4 rounded border-gray-300 dark:border-surface-tonal-a30 text-gray-900 focus:ring-gray-500 transition-all">
                                        <div class="ml-3">
                                            <h3 class="text-sm font-semibold text-gray-900 dark:text-white">Featured Content</h3>
                                ...
                                <div class="space-y-2">
                                    ...
                                    <div class="aspect-[16/9] admin-upload-zone"
                                        ...
                                        <img id="banner-preview" src="{{ $collection->banner_url }}" class="absolute inset-0 w-full h-full object-cover {{ $collection->banner_url ? '' : 'hidden' }} z-10">
                                        <div id="banner-placeholder" class="admin-upload-placeholder {{ $collection->banner_url ? 'hidden' : '' }}">
                                            <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                            </svg>
                                            <p>Change Banner Image</p>
                                        </div>
                                        <input type="file" name="banner_image" id="banner-input" class="hidden" accept="image/*" onchange="previewBanner(this)">
                                    </div>
