    ...
    <div class="px-4 sm:px-6 lg:px-4 pb-4 sm:py-12 pt-20">
        <div class=" flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            ...
            <div class="flex flex-row-reverse sm:flex-row gap-3 w-full sm:w-auto justify-between sm:justify-end">
                ...
                <a href="{{ route('prescriptions.create', ['from' => 'doctor-panel']) }}"
        ...
        <div id="bulk-delete-form" class="hidden mb-6">
            <form method="POST" action="{{ route('prescriptions.bulkDelete') }}" id="bulk-delete-form-element"
            ...
            <div class="fixed inset-y-0 right-0 w-full max-w-md bg-white dark:bg-gray-800 shadow-xl transform translate-x-full transition-transform duration-300 ease-in-out"
                ...
                <div class="p-6 overflow-y-auto h-full pb-32">
                    <div class="space-y-6">
                        <div>
                            <label
                                class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">{{ __('file.type') }}</label>
        ...
        <script>
            document.addEventListener('DOMContentLoaded', function () {
                ...
                const table = $('#docapp-table').DataTable({
                    ...
                    responsive: false,
                    ajax: {
                        url: '{{ route('doctor-panel.prescriptions.datatable') }}',
                        data: function (d) {
                        ...
                        { data: 'medications_count', className: 'text-center', render: data => `<span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-indigo-100 dark:bg-indigo-900/30 text-indigo-800 dark:text-indigo-300">${data}</span>` },
                        {
                            ...
                            render: (data, type, row) => `
                                                <div class="flex items-center justify-end gap-1">
                                                    <a href="${row.print_url}" class="p-2 text-gray-600 dark:text-gray-400 hover:text-green-600 dark:hover:text-green-400 transition-colors" title="{{ __('file.print') }}">
                                                    ...
                                                    <a href="${row.edit_url || '#'}" class="p-2 ${row.edit_url ? 'text-gray-600 dark:text-gray-400 hover:text-indigo-600 dark:hover:text-indigo-400' : 'text-gray-400 dark:text-gray-600 opacity-50 cursor-not-allowed'} transition-colors" ${!row.edit_url ? 'disabled' : ''}>
                                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                                                    </a>
                                                    ${row.delete_url ? `
                                                        <form method="POST" action="${row.delete_url}" class="inline">
                                                            @csrf @method('DELETE')
                                                            <button type="submit" onclick="return confirm('{{ __('file.confirm_delete') }}')" class="p-2 text-gray-600 dark:text-gray-400 hover:text-red-600 dark:hover:text-red-500 transition-colors">
