...
@section('content')
    <div x-data="pickupManager()" @edit-pickup.window="openEditModal($event.detail)"
        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div>
                <h1 class="text-2xl sm:text-3xl font-semibold text-gray-900 dark:text-primary-a0">Pickup Locations</h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Offer physical pickup points as alternatives to
                    home delivery.</p>
            </div>
            <div>
        ...
        @if(session('success'))
            <div class="mb-4 bg-green-50 border-l-4 border-green-500 p-4 rounded-md text-sm text-green-700">
                {{ session('success') }}
            </div>
        @endif
        @if($errors->any())
        ...
        <div id="bulk-delete-form" class="hidden mb-6">
            <form method="POST" action="{{ route('shipping.pickups.bulkDelete') }}" id="bulk-delete-form-el"
                ...
                <span class="text-sm font-medium text-red-800 dark:text-red-300">
                    <span id="selected-count">0</span> locations selected
                </span>
                <button type="submit"
        ...
        <div x-show="isModalOpen" style="display: none;" class="fixed inset-0 z-50 overflow-y-auto">
            <div class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div x-show="isModalOpen" x-transition
                    class="inline-block align-bottom bg-white dark:bg-surface-tonal-a20 rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-xl w-full">
                    <form :action="formUrl" method="POST">
                        ...
                        <div
                            ...
                            <h3 class="text-xl leading-6 font-bold text-gray-900 dark:text-primary-a0"
                                x-text="isEditing ? 'Edit Location' : 'Add Pickup Location'"></h3>
                            ...
                            <div class="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <div class="sm:col-span-2 mt-2">
                                    <label class="inline-flex items-center">
                                        ...
                                        <span class="ml-2 text-sm text-gray-700 dark:text-gray-300">Location available for
                                            pickup at checkout</span>
                                    </label>
                                </div>
                            </div>
                        </div>
        ...
        <script>
            document.addEventListener('alpine:init', () => {
                Alpine.data('pickupManager', () => ({
                    isModalOpen: false,
                    isEditing: false,
                    storeUrl: '{{ route("shipping.pickups.store") }}',
                    updateBaseUrl: '{{ route("shipping.pickups.update", ":id") }}',
                    ...
                    openCreateModal() {
                        ...
                        this.form = { id: null, name: '', address_line_1: '', address_line_2: '', city: '', state: '', postal_code: '', country: 'LK', phone: '', email: '', is_active: true };
                    ...
                    openEditModal(loc) {
                        this.isEditing = true;
                        this.form = { ...loc };
                        this.form.is_active = loc.is_active == 1; // Parse boolean
                        this.isModalOpen = true;
                    }
                }));
            });
            ...
            document.addEventListener('DOMContentLoaded', function () {
                const table = $('#application-table').DataTable({
                    ...
                    responsive: false,
                    ajax: {
                        url: '{{ route('shipping.pickups.datatable') }}'
                    },
                    order: [[1, 'asc']],
                    columnDefs: [
                    ...
                    columns: [
                        ...
                        { data: 'name', name: 'name', className: 'whitespace-nowrap py-4 pl-4 pr-3 text-sm font-bold text-gray-900 dark:text-primary-a0 sm:pl-6' },
                        { data: 'address_html', searchable: false, orderable: false, className: 'px-3 py-4 text-sm text-gray-500 dark:text-gray-400' },
                        { data: 'contact_html', searchable: false, orderable: false, className: 'px-3 py-4 text-sm text-gray-500 dark:text-gray-400' },
                    ...
                    language: {
                        ...
                        emptyTable: "No pickup locations configured.",
                        processing: false,
                    },
                    autoWidth: false,
                    scrollX: false
                ...
                window.confirmDelete = function (url) {
                    if (!confirm('Are you sure you want to completely remove this location?')) return;
