        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div>
                <h1 class="text-2xl sm:text-3xl font-semibold text-gray-900 dark:text-primary-a0">
                    {{ __('file.brands') ?? 'Brands' }}
                </h1>
                <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                    {{ __('file.manage_brands') ?? 'Manage your store brands' }}
                </p>
            </div>
        ...
        <div id="bulk-delete-form" class="hidden mb-6">
            <form method="POST" action="{{ route('brands.bulkDelete') }}" id="bulk-delete-form-el"
                ...
                <span class="text-sm font-medium text-red-800 dark:text-red-300">
                    <span id="selected-count">0</span> {{ __('file.brands_selected') ?? 'brands selected' }}
                </span>
                <button type="submit"
                ...
                <table id="application-table" class="w-full divide-y divide-gray-200 dark:divide-surface-tonal-a30 nowrap" style="width:100%">
                    <thead class="bg-gray-50 dark:bg-surface-tonal-a10 border-b border-gray-200 dark:border-surface-tonal-a30">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider all">
                                Logo</th>
                            <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider all">
                                {{ __('file.name') ?? 'Name' }}</th>
                            <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider desktop">
                                {{ __('file.sort_order') ?? 'Sort Order' }}</th>
                            <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider desktop">
                                Products</th>
                            <th scope="col" class="px-6 py-4 text-center text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider desktop">
                                {{ __('file.status') ?? 'Status' }}</th>
                            <th scope="col" class="px-6 py-4 text-right text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider all">
                                {{ __('file.actions') ?? 'Actions' }}</th>
                        </tr>
                    </thead>
                    <tbody class="bg-white dark:bg-surface-tonal-a20 divide-y divide-gray-200 dark:divide-gray-700 [&>tr]:group">
                    </tbody>
                </table>
            </div>
        </div>
            ...
            document.addEventListener('DOMContentLoaded', function () {
                const table = $('#application-table').DataTable({
                    ...
                    columns: [
                        ...
                        { data: 'products_html', name: 'products_count', searchable: false },
                        { data: 'status_html', name: 'status', className: 'text-center', searchable: false },
                        {
                            data: null,
                            orderable: false,
                            searchable: false,
                    ...
                    language: {
                        ...
                        emptyTable: "No brands found matching your criteria.",
                        processing: false,
                    },
                    autoWidth: false,
                    scrollX: false
