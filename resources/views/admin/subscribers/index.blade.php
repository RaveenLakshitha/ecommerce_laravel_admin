    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <h1 class="text-2xl sm:text-3xl font-semibold text-gray-900 dark:text-primary-a0">
                {{ __('file.subscribers') ?? 'Newsletter Subscribers' }}
            </h1>
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">
                {{ __('file.manage_subscribers') ?? 'Monitor marketing opt-ins and acquisition source attribution' }}
            </p>
        </div>
    </div>
            ...
            <table id="application-table" class="w-full divide-y divide-gray-200 dark:divide-surface-tonal-a30 nowrap" style="width:100%">
                <thead class="bg-gray-50 dark:bg-surface-tonal-a10 border-b border-gray-200 dark:border-surface-tonal-a30">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider all">Email Address</th>
                        <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider desktop">Name</th>
                        <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider desktop">Source</th>
                        ...
                        <th scope="col" class="px-6 py-4 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wider desktop">Subscribed At</th>
        ...
        const table = $('#application-table').DataTable({
            ...
            columns: [
                ...
                { 
                    ...
                    render: data => `<span class="text-sm font-medium text-gray-900 dark:text-white font-mono">${data}</span>`
                },
                { 
                    data: 'name', name: 'first_name',
                    render: data => `<span class="text-sm text-gray-600 dark:text-gray-400">${data || '—'}</span>`
                },
                { 
                    data: 'source', name: 'source',
                    render: data => `<span class="px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-gray-100 text-gray-600 dark:bg-surface-tonal-a30 dark:text-gray-400 border border-gray-200 dark:border-surface-tonal-a40">${data || 'Organic'}</span>`
                },
                { data: 'status_html', name: 'status', className: 'text-center' },
                { 
                    data: 'subscribed_at', name: 'subscribed_at',
                    render: data => `<span class="text-xs text-gray-500 dark:text-gray-400">${data}</span>`
                },
                {
                    data: null,
                    orderable: false,
