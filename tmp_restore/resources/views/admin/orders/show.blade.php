        ...
        <div class="max-w-[1400px] mx-auto">
            ...
            <div class="mb-8 flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4">
                <div>
                    ...
                    <div class="flex items-center gap-4 mt-2">
                        <h1 class="text-3xl font-bold tracking-tight text-gray-900 dark:text-white">{{ $order->order_number }}</h1>
                        @php
                            $statusMap = [
                        ...
                        @endphp
                        <span class="px-2.5 py-0.5 rounded-full text-[10px] font-black uppercase tracking-widest border {{ $s['class'] }}">
                            {{ $s['label'] }}
                        </span>
                    </div>
            ...
            <div class="grid grid-cols-1 xl:grid-cols-3 gap-8">
                ...
                <div class="xl:col-span-2 space-y-6">
                    ...
                    <div
                        ...
                        <div
                            ...
                            <span class="text-[10px] font-bold text-indigo-500 uppercase tracking-widest">{{ $order->items->count() }} Items</span>
                        </div>
                        <div class="overflow-x-auto">
                            <table class="w-full text-left border-collapse">
                                <thead>
                                    <tr class="bg-gray-50/50 dark:bg-surface-tonal-a10/50 border-b border-gray-100 dark:border-surface-tonal-a30">
                                        <th class="px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest">Product</th>
                                        <th class="px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest text-right">Price</th>
                                        <th class="px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest text-center">Qty</th>
                                        <th class="px-6 py-4 text-[10px] font-bold text-gray-400 uppercase tracking-widest text-right">Total</th>
                                    </tr>
                                </thead>
                                <tbody class="divide-y divide-gray-50 dark:divide-surface-tonal-a30">
                                    @foreach($order->items as $item)
                                        <tr class="hover:bg-gray-50 dark:hover:bg-surface-tonal-a30/20 transition-colors">
                                            <td class="px-6 py-4">
                                                <div class="text-sm font-bold text-gray-700 dark:text-white underline decoration-indigo-500/30 decoration-2 underline-offset-4">{{ $item->product_name_snapshot }}</div>
                                                @if($item->variant_attributes)
                                                    <div class="flex flex-wrap gap-1 mt-1.5">
                                                        @foreach(json_decode($item->variant_attributes, true) ?? [] as $key => $val)
                                                            <span class="px-1.5 py-0.5 rounded bg-gray-100 dark:bg-surface-tonal-a30 text-gray-500 dark:text-gray-400 text-[8px] font-bold border border-gray-200 dark:border-surface-tonal-a30 uppercase tracking-tighter">
                                                                {{ $key }}: {{ $val }}
                                                            </span>
                                                        @endforeach
                                                    </div>
                                            ...
                                            <td class="px-6 py-4 text-right">
                                                <span class="text-xs font-mono text-gray-500 dark:text-gray-400">{{ $order->currency }} {{ number_format($item->unit_price, 2) }}</span>
                                            </td>
                                            <td class="px-6 py-4 text-center">
                                                <span class="inline-flex items-center justify-center w-6 h-6 rounded-lg bg-indigo-50 dark:bg-indigo-900/20 text-indigo-600 dark:text-indigo-400 text-[10px] font-black border border-indigo-100 dark:border-indigo-900/30">
                                                    {{ $item->quantity }}
                                                </span>
                                            </td>
                                            <td class="px-6 py-4 text-right">
                                                <span class="text-sm font-bold text-gray-900 dark:text-white font-mono tabular-nums">{{ $order->currency }} {{ number_format($item->total, 2) }}</span>
                                            </td>
                                        </tr>
                                    @endforeach
                                </tbody>
                        ...
                        <div class="p-6 bg-gray-50/50 dark:bg-surface-tonal-a20/50 border-t border-gray-100 dark:border-surface-tonal-a30">
                                ...
                                <div class="w-full sm:w-64 space-y-3">
                                    ...
                                    @if($order->discount_amount > 0)
                                        <div class="flex justify-between items-center text-xs font-bold text-red-500">
                                            ...
                                            <span class="font-mono tabular-nums">-{{ $order->currency }} {{ number_format($order->discount_amount, 2) }}</span>
                                        </div>
                                    @endif
                                    ...
                                    @endif
                                    <div class="pt-3 border-t border-gray-200 dark:border-surface-tonal-a30 flex justify-between items-center">
                                        <span class="text-sm font-black text-gray-900 dark:text-white uppercase tracking-widest">Total</span>
                                        <span class="text-xl font-black text-indigo-600 dark:text-indigo-400 font-mono tabular-nums">{{ $order->currency }} {{ number_format($order->total_amount, 2) }}</span>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    @if($order->refunds->count() > 0)
                        <div class="bg-red-50/30 dark:bg-red-950/10 rounded-2xl border border-red-100 dark:border-red-950/20 overflow-hidden">
                                ...
                                <table class="w-full text-left">
                                    <tbody class="divide-y divide-red-100/50 dark:divide-red-950/30">
                                        @foreach($order->refunds as $refund)
                                            <tr class="text-xs">
                                                <td class="px-6 py-4 font-bold text-red-600 dark:text-red-400 uppercase tracking-widest">{{ $refund->created_at->format('M d, Y') }}</td>
                                                <td class="px-6 py-4 text-gray-600 dark:text-gray-300">{{ $refund->reason ?: 'No reason' }}</td>
                                                <td class="px-6 py-4 text-right font-black text-red-600 dark:text-red-400">-${{ number_format($refund->amount, 2) }}</td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                    ...
                    @if($refundableAmount > 0)
                        <div class="bg-white dark:bg-surface-tonal-a20 rounded-2xl shadow-sm border border-gray-200 dark:border-surface-tonal-a30 p-6">
                            <h3 class="font-bold text-gray-900 dark:text-white mb-6 flex items-center gap-2">
                                <span class="w-8 h-8 rounded-lg bg-red-500 text-white flex items-center justify-center text-lg">↺</span>
                                Process Marketplace Refund
                            </h3>
                            <form action="{{ route('orders.refund', $order->id) }}" method="POST" class="grid grid-cols-1 sm:grid-cols-3 gap-6 items-end">
                                @csrf
                                <div class="space-y-1.5">
                                    <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest">Amount ({{ $order->currency }})</label>
                                    <input type="number" step="0.01" name="amount" min="0.01" max="{{ $refundableAmount }}" value="{{ $refundableAmount }}" required
                                    ...
                                    <p class="text-[9px] text-gray-400 font-medium">Max possible: ${{ number_format($refundableAmount, 2) }}</p>
                                </div>
                                <div class="space-y-1.5 sm:col-span-1">
                                    <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest">Reason</label>
                ...
                <div class="xl:col-span-1 space-y-6">
                    ...
                    <div class="bg-white dark:bg-surface-tonal-a20 rounded-2xl shadow-sm border border-gray-200 dark:border-surface-tonal-a30 overflow-hidden">
                        ...
                        <div class="p-6">
                            <form action="{{ route('orders.update-status', $order->id) }}" method="POST" class="space-y-4">
                                ...
                                @method('PATCH')
                                <div class="space-y-1.5">
                                    <label class="block text-[10px] font-bold text-gray-400 uppercase tracking-widest">Order Status</label>
                                    <select name="status" class="block w-full rounded-xl border-gray-200 dark:border-surface-tonal-a30 bg-white dark:bg-surface-tonal-a30 px-4 py-3 text-xs font-bold text-gray-700 dark:text-gray-200 focus:ring-2 focus:ring-indigo-500 transition-all">
                                        @foreach(['pending', 'processing', 'shipped', 'delivered', 'cancelled', 'returned'] as $st)
                                            <option value="{{ $st }}" {{ $order->status === $st ? 'selected' : '' }}>{{ ucfirst($st) }}</option>
                                        @endforeach
                                    </select>
                                </div>
                                <button type="submit" class="w-full flex items-center justify-center px-4 py-3 rounded-xl bg-gray-900 dark:bg-white text-[10px] font-black text-white dark:text-gray-900 uppercase tracking-widest hover:bg-black dark:hover:bg-gray-100 transition-all shadow-lg active:scale-95">
                                    Transition Status
                                </button>
                            </form>
                        </div>
                    ...
                    <div class="bg-white dark:bg-surface-tonal-a20 rounded-2xl shadow-sm border border-gray-200 dark:border-surface-tonal-a30 p-6">
                        <h3 class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-4">Customer Identity</h3>
                        <div class="flex items-center gap-4 mb-6">
                            <div class="w-12 h-12 rounded-2xl bg-indigo-50 dark:bg-indigo-900/20 flex items-center justify-center text-lg font-black text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-900/30">
                                {{ strtoupper(substr($order->customer_name, 0, 1)) }}
                            </div>
                            <div class="flex flex-col">
                                <span class="text-sm font-black text-gray-900 dark:text-white leading-tight underline decoration-indigo-500/30 decoration-2 underline-offset-2">{{ $order->customer_name }}</span>
                                <span class="text-[10px] font-bold text-gray-400 mt-0.5 truncate max-w-[150px]">{{ $order->customer_email }}</span>
                            </div>
                            @if($order->customer_id)
                                <a href="{{ route('customers.show', $order->customer_id) }}" class="ml-auto p-2 rounded-xl bg-gray-50 dark:bg-surface-tonal-a30 text-gray-400 hover:text-indigo-500 transition-all">
                        ...
                        <div class="space-y-4 pt-4 border-t border-gray-50 dark:border-surface-tonal-a30">
                            <div>
                                ...
                                <p class="text-xs font-bold text-gray-700 dark:text-gray-300">{{ $order->customer_phone ?? '—' }}</p>
                            </div>
                        </div>
                    </div>

                    {{-- Logistic Destination Card --}}
                    <div class="bg-white dark:bg-surface-tonal-a20 rounded-2xl shadow-sm border border-gray-200 dark:border-surface-tonal-a30 p-6 relative overflow-hidden group">
                        <div class="absolute top-0 right-0 p-4 opacity-5 group-hover:rotate-12 transition-transform duration-500">
                        ...
                        <h3 class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-4">Logistic Destination</h3>
                        @if($order->shippingAddress)
                            <div class="space-y-1 relative z-10">
                                <p class="text-sm font-black text-gray-900 dark:text-white">{{ $order->shippingAddress->first_name }} {{ $order->shippingAddress->last_name }}</p>
                                <p class="text-xs font-medium text-gray-500 dark:text-gray-400 leading-relaxed">
                                    ...
                                    <span class="font-black text-indigo-500 uppercase tracking-tighter">{{ $order->shippingAddress->country }}</span>
                                </p>
                            </div>
                        @else
                    ...
                    <div class="bg-white dark:bg-surface-tonal-a20 rounded-2xl shadow-sm border border-gray-200 dark:border-surface-tonal-a30 p-6">
                        <h3 class="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-4">Staff Annotations</h3>
                        <form action="{{ route('orders.notes.add', $order->id) }}" method="POST" class="space-y-4">
                            ...
                            <button type="submit" class="w-full h-10 flex items-center justify-center rounded-xl bg-gray-50 dark:bg-surface-tonal-a30 text-gray-600 dark:text-gray-400 text-[10px] font-black uppercase tracking-widest hover:bg-gray-100 dark:hover:bg-gray-700 transition-all border border-gray-100 dark:border-surface-tonal-a30">
                                Commit Annotation
                            </button>
                        </form>
                    </div>
