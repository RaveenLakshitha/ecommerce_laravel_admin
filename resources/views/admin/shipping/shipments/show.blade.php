        ...
        <div class="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 mb-8 no-print">
            <div>
                <div class="flex items-center gap-4">
                    ...
                    <span class="admin-badge admin-badge-neutral text-xs font-bold uppercase tracking-wider italic">
                        ...
                        <form action="{{ route('shipping.shipments.update', $shipment) }}" method="POST">
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
                                <div>
                                     ...
                                     <select name="courier_id" class="block w-full rounded-md border border-gray-200 dark:border-white/10 bg-white dark:bg-surface-tonal-a20 px-3 py-2.5 text-sm font-semibold text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500/30 cursor-pointer transition-all">
                                 ...
                                 <div>
                                     ...
                                     <select name="status" class="block w-full rounded-md border border-gray-200 dark:border-white/10 bg-white dark:bg-surface-tonal-a20 px-3 py-2.5 text-sm font-semibold text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500/30 cursor-pointer transition-all">
                                    ...
                                    <div class="relative">
                                        <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-400">
                                         ...
                                         <input type="text" name="tracking_number" value="{{ old('tracking_number', $shipment->tracking_number) }}" placeholder="e.g. TRK-XXXX-YYYY-ZZ"
                                             class="block w-full rounded-md border border-gray-200 dark:border-white/10 bg-white dark:bg-surface-tonal-a20 pl-9 pr-3 py-2.5 text-sm font-mono font-bold text-indigo-600 dark:text-indigo-400 outline-none focus:ring-2 focus:ring-indigo-500/30 shadow-sm transition-all uppercase">
                                    </div>
                                </div>
                                ...
                                <div class="sm:col-span-2">
                                    <label class="block text-xs font-semibold text-gray-700 dark:text-gray-300 uppercase tracking-wider mb-1">{{ __('file.logistics_annotation_internal') }}</label>
                                     <textarea name="notes" rows="3" placeholder="{{ __('file.append_transit_observations_placeholder') }}"
                                         class="block w-full rounded-md border border-gray-200 dark:border-white/10 bg-white dark:bg-surface-tonal-a20 px-3 py-2.5 text-sm font-medium text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500/30 transition-all resize-none leading-relaxed">{{ old('notes', $shipment->notes) }}</textarea>
                                </div>
                            </div>
                                                ...
                                                <div>
                                                    <div class="flex items-center gap-2">
                                                        <p class="text-xs font-bold text-gray-900 dark:text-white uppercase tracking-wider">{{ __('file.' . $event->status) }}</p>
                                                         @if($event->location)
                                                             <span class="px-2 py-0.5 rounded-md text-xs font-bold uppercase tracking-wider bg-indigo-50 dark:bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border border-indigo-100 dark:border-indigo-500/20 italic">{{ $event->location }}</span>
                                                         @endif
                                                    </div>
                        ...
                        <div class="mt-10 pt-6 border-t border-gray-100 dark:border-surface-tonal-a30">
                            ...
                            <form action="{{ route('shipping.shipments.tracking', $shipment) }}" method="POST" class="bg-gray-50 dark:bg-surface-tonal-a10 p-4 rounded-xl border border-gray-100 dark:border-surface-tonal-a30 space-y-4">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                     <div>
                                         <label class="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5">{{ __('file.event_status') }}</label>
                                         <select name="status" class="block w-full rounded-md border border-gray-200 dark:border-white/10 bg-white dark:bg-surface-tonal-a20 px-3 py-2.5 text-sm font-semibold text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500/30 transition-all cursor-pointer" required>
                                     ...
                                     <div>
                                         <label class="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5">{{ __('file.geo_location_node') }}</label>
                                         <input type="text" name="location" placeholder="e.g. CMB-HUB-01" 
                                             class="block w-full rounded-md border border-gray-200 dark:border-white/10 bg-white dark:bg-surface-tonal-a20 px-3 py-2.5 text-sm font-mono font-bold text-indigo-600 dark:text-indigo-400 outline-none focus:ring-2 focus:ring-indigo-500/30 transition-all uppercase">
                                     </div>
                                 </div>
                                 <div>
                                     <label class="block text-xs font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-1.5">{{ __('file.protocol_details') }}</label>
                                     <input type="text" name="description" placeholder="{{ __('file.specify_precise_event_observations_placeholder') }}" required 
                                         class="block w-full rounded-md border border-gray-200 dark:border-white/10 bg-white dark:bg-surface-tonal-a20 px-3 py-2.5 text-sm font-medium text-gray-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500/30 transition-all">
                                 </div>
            ...
            <div class="col-span-1 space-y-4">
                ...
                <div class="bg-white dark:bg-surface-tonal-a20 rounded-lg shadow-sm border border-gray-200 dark:border-surface-tonal-a30 p-4">
                    ...
                    <div class="flex flex-col gap-1">
                        ...
                        <p class="text-xs font-semibold text-indigo-600 dark:text-indigo-400 truncate">{{ $shipment->order->customer?->email }}</p>
                        <p class="text-xs font-medium text-gray-500 mt-1 italic">{{ $shipment->order->customer?->phone }}</p>
                    </div>
                        ...
                        @if($shipment->pickupLocation)
                            <div class="p-3 rounded-lg bg-blue-500/5 border border-blue-500/10">
                                <div class="flex items-center gap-2 mb-2">
                                    ...
                                    <span class="text-xs font-bold text-blue-600 dark:text-blue-400 uppercase tracking-wider">{{ __('file.protocol_in_hub_pickup') }}</span>
                                </div>
                                <p class="text-xs font-black text-gray-900 dark:text-white leading-tight">{{ $shipment->pickupLocation->name }}</p>
                                <p class="text-xs font-medium text-gray-500 dark:text-gray-400 mt-1 leading-relaxed">{{ $shipment->pickupLocation->address_line_1 }}<br>{{ $shipment->pickupLocation->city }}</p>
                            </div>
                        @else
                            @if($shipment->order->deliveryAddress)
                                <div class="p-3 rounded-lg bg-gray-50 dark:bg-surface-tonal-a10 border border-gray-100 dark:border-surface-tonal-a30">
                                    <div class="flex items-center gap-2 mb-3">
                                        ...
                                        <span class="text-xs font-semibold text-indigo-600 dark:text-indigo-400 uppercase tracking-wider">{{ __('file.protocol_direct_fulfillment') }}</span>
                                    </div>
                                    <div class="space-y-1">
                                        ...
                                        @if($shipment->order->deliveryAddress->address_line_2)
                                            <p class="text-xs font-semibold text-gray-500 dark:text-gray-400">{{ $shipment->order->deliveryAddress->address_line_2 }}</p>
                                        @endif
                                        <p class="text-md font-bold text-indigo-500 dark:text-indigo-400 uppercase tracking-widest pt-1">{{ $shipment->order->deliveryAddress->city }}, {{ $shipment->order->deliveryAddress->postal_code }}</p>
                            ...
                            @else
                                <div class="p-3 rounded-lg bg-rose-500/5 border border-rose-500/10 flex items-center gap-3">
                                    ...
                                    <span class="text-xs font-black text-rose-600 dark:text-rose-400 uppercase tracking-widest leading-tight">{{ __('file.geocoding_logic_error_no_destination') }}</span>
                                </div>
                            @endif
                        @endif
                    </div>
        ...
        <div id="printable-waybill" class="hidden">
            ...
            <div class="border-2 border-black p-6 mb-10">
                 <div class="grid grid-cols-3 gap-4">
                     <div>
                         <h4 class="text-xs font-black uppercase text-gray-500">{{ __('file.order_reference') }}</h4>
                         <p class="text-lg font-black">#{{ str_pad($shipment->order->id, 5, '0', STR_PAD_LEFT) }}</p>
                     </div>
                     <div>
                         <h4 class="text-xs font-black uppercase text-gray-500">{{ __('file.target_courier') }}</h4>
                         <p class="text-lg font-black uppercase">{{ $shipment->courier?->name ?? __('file.in_store_pickup') }}</p>
                     </div>
                     <div>
                         <h4 class="text-xs font-black uppercase text-gray-500">{{ __('file.carrier_tracking_protocol') }}</h4>
                        ...
                        @foreach($shipment->order->items as $item)
                         <tr class="border-b border-gray-200">
                             <td class="py-3">
                                 ...
                                 <p class="text-xs text-gray-500 font-bold uppercase">{{ $item->variant_name }}</p>
                             </td>
