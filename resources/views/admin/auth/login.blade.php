<x-admin-guest-layout>
    ...
    <div class="lg:hidden flex flex-col items-center mb-8 text-center pt-8">
        <div
            ...
            @if($site_logo ?? false)
                <img src="{{ $site_logo }}" alt="{{ $site_name ?? __('messages.Admin Portal') }} Logo"
        ...
        <h1 class="text-2xl font-bold text-slate-800">
            {{ $site_name ?? __('messages.Admin Portal') }}
        </h1>
    </div>
    ...
    <form method="POST" action="{{ route('admin.login') }}" class="space-y-6">
        ...
        <div class="space-y-2">
            <div class="flex items-center justify-between">
                <label for="password"
                    class="block text-sm font-medium text-slate-700">{{ __('messages.Password') }}</label>
                @if (Route::has('password.request'))
                    <a href="{{ route('password.request') }}"
                        class="text-sm font-semibold text-sky-600 hover:text-sky-500 transition-colors">
        ...
        <div class="flex items-center">
            <label for="remember_me" class="inline-flex items-center cursor-pointer group">
                <div class="relative flex items-center">
                    ...
                    <svg class="absolute w-3.5 h-3.5 text-white left-1/2 top-1/2 transform -translate-x-1/2 -translate-y-1/2 opacity-0 peer-checked:opacity-100 pointer-events-none"
                        fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="3" d="M5 13l4 4L19 7"></path>
                    </svg>
                </div>
                <span
                    class="ml-3 text-sm font-medium text-slate-600 group-hover:text-slate-800 transition-colors">{{ __('messages.Remember me this device') }}</span>
            </label>
        </div>
        ...
        <div class="pt-2">
            <button type="submit"
                ...
                <span>{{ __('messages.Sign in to Admin Dashboard') }}</span>
